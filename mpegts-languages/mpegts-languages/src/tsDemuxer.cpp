...
int AVContext::configure_ts(pthread_t threadId)
{
	...
	{
		const unsigned char* data = m_demux->ReadAV(pos, data_size, &result_size);
		
#ifdef DBG_TSDEMUXER
		GetTimestamp((char *) &timestamp);
		sprintf(logBuf, "%12lu %s AVContext::configure_ts(1) ReadAV", threadId, timestamp);
...
#ifdef DBG_TSDEMUXER
						GetTimestamp((char *) &timestamp);
						sprintf(logBuf, "%12lu %s AVContext::configure_ts(2) ReadAV", threadId, timestamp);
						Logger::instance().log(logBuf, Logger::kLogLevelInfo);
						// printf("%s\n", logBuf);
#endif
						
						if (!(ndata = m_demux->ReadAV(npos, data_size, &result_size)))
							return AVCONTEXT_IO_ERROR;
	...
	Logger::instance().log("Invalid stream", Logger::kLogLevelError);
	
	return AVCONTEXT_TS_NOSYNC;
}

int AVContext::TSResync(pthread_t threadId)
{
	if (!is_configured)
	{
		int ret = configure_ts(threadId);
	...
	for (int i = 0; i < MAX_RESYNC_SIZE; i++)
	{
		const unsigned char* data = m_demux->ReadAV(av_pos, av_pkt_size, &result_size);

#ifdef DBG_TSDEMUXER
		GetTimestamp((char *) &timestamp);
		sprintf(logBuf, "%12lu %s AVContext::TSResync() ReadAV", threadId, timestamp);
		Logger::instance().log(logBuf, Logger::kLogLevelInfo);
		// printf("%s\n", logBuf);
#endif 
		if (result_size < 0)
 ...
 *  Parsing error !
 */
int AVContext::ProcessTSPacket(uint16_t *tsPid)
{
	PLATFORM::CLockObject lock(mutex);
	...
	*tsPid = pid;
	...
	else
	{
		...
		if(it->second.wait_unit_start && !this->payload_unit_start)
		{
			...
			// DBG(DEMUX_DBG_WARN, "PID %.4x discontinuity detected: wait_unit_start %s, payload_unit_start %s\n", this->pid, it->second.wait_unit_start ? "true" : "false", this->payload_unit_start ? "true" : "false");
			// printf("x ");
		...
		if(it->second.continuity != 0xff)
		{
			...
			if (!is_discontinuity && expected_cc != continuity_counter)
			{
				...
				if(!this->payload_unit_start)
				{
					...
					uint8_t c1 = continuity_counter;
					uint8_t c2 = expected_cc;
					uint8_t difference;
					
					if (continuity_counter > expected_cc)
						difference = continuity_counter - expected_cc;
					else
					{
						difference = 16 - expected_cc;
						difference += continuity_counter;
					}
					
					// DBG(DEMUX_DBG_WARN, "PID %.4x discontinuity detected: found %u, expected %u, difference %u\n", this->pid, continuity_counter, expected_cc, difference);
					return AVCONTEXT_DISCONTINUITY;
				}
			}
		}
	...
	case 0x02: // parse PMT table
		{
			...
			while (psi < end_psi)
			{
				...
				DBG(DEMUX_DBG_DEBUG,
					"%s: PMT(%.4x) version %u: new PES %.4x %s\n",
					__FUNCTION__,
					this->packet->pid,
					version,
					pes_pid,
					ElementaryStream::GetStreamCodecName(stream_type));
				if (stream_type != STREAM_TYPE_UNKNOWN)
				{
					...
					pes.stream = es;
					DBG(DEMUX_DBG_DEBUG,
						"%s: PMT(%.4x) version %u: register PES %.4x %s\n",
						__FUNCTION__,
						this->packet->pid,
						version,
						pes_pid,
