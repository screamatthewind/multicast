...
int k_req_incoming2(uint32_t source, struct rpfctl *rpf)
{
	int l, rlen;
	char buf[512];
...
//	logit(LOG_DEBUG, 0, "-----");
	...
	if (strcmp(s1, "10.153.128.1") == 0)	
		logit(LOG_DEBUG, 0, "rdl: k_req_incoming - source %s", s1);
	...
	int result = getmsg(NLMSG_DATA(n), l - sizeof(*n), rpf);
	
	logit(LOG_DEBUG,
		0,
		"rdl: k_req_incoming-3: source %s, iif %d",
		s1, rpf->iif);
...
int k_req_incoming(uint32_t source, struct rpfctl *rpf) {
	...
	do {
		result = k_req_incoming2(source, rpf);
		if (i++ > numvifs)
			break;
	} while (rpf->iif == NO_VIF);
	

	logit(LOG_DEBUG, 0, "rdl: k_req_incoming-4: source %s, iif %d", s1, rpf->iif);
	logit(LOG_DEBUG, 0, "-----");
...
static int getmsg(struct rtmsg *rtm, int msglen, struct rpfctl *rpf)
{
	...
	ifindex = *(int *)(((char*)(rta[RTA_OIF])) + RTA_LENGTH(0));
	...
	inet_fmt(rpf->source.s_addr, s2, sizeof(s2));
	
	if (strcmp(s1, "10.153.128.1") == 0) {
		logit(LOG_DEBUG, 0, "rdl: k_req_incoming - source %s, ifindex %d, vifi %d", s1, ifindex, vifi);
//		dump_route_table2();
	}
