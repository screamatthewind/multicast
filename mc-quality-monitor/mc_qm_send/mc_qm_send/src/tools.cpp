...
char *get_ip_address(char *if_name)
{
	char *result = NULL;
	ps_interface_t *ps_interface, *ps_interface_tmp;
...
int inet_cksum(unsigned char *addr, u_int len)
{
	int sum = 0;
	int nleft = (int)len / sizeof(uint16_t);
...
unsigned char *rand_string(unsigned char *str, size_t size)
{
