...
void CsvWriter::OpenStream()
{
	...
	if (strcmp(hostname, "(none)") == 0)
		sprintf(filename, "/tmp/mc_qm_recv-%s.csv", filename_timestamp);
	else	
		sprintf(filename, "/tmp/%s-mc_qm_recv-%s.csv", hostname, filename_timestamp);
...
void CsvWriter::write(const vector<string>& inMessages)
{
	...
	for (size_t i = 0; i < inMessages.size(); i++) {
		writeHelper(inMessages[i], false);
	}
}
