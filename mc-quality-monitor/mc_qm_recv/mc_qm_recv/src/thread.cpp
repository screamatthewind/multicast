...
static bool GetAbsTimeUs(struct timespec *Abstime, int MicrosecondsFromNow)
{
	struct timeval now;
	if (gettimeofday(&now, NULL) == 0) {
		// get current time
			now.tv_sec  += MicrosecondsFromNow / 1000000;     // add full seconds
			now.tv_usec += MicrosecondsFromNow % 1000000;       // add microseconds
			if(now.tv_usec >= 1000000) {
						// take care of an overflow
			now.tv_sec++;
			now.tv_usec -= 1000000;
...
{
	cCondWait w;
	w.Wait(max((int) TimeoutMs, (int) 3));  // making sure the time is >2ms to avoid a possible busy wait
}
...
bool cCondWait::WaitUs(int TimeoutUs)
{
	pthread_mutex_lock(&mutex);
	if (!signaled) {
		if (TimeoutUs) {
			struct timespec abstime;
			if (GetAbsTimeUs(&abstime, TimeoutUs)) {
				while (!signaled) {
					if (pthread_cond_timedwait(&cond, &mutex, &abstime) == ETIMEDOUT)
						break;
				}
