...
//	strftime(filename_timestamp, LINESZ, "%m%d%Y_%H%M", localtime(&now));
	strftime(filename_timestamp, LINESZ, "%m%d%Y", localtime(&now));

	gethostname(hostname, LINESZ);
	if (strcmp(hostname, "(none)") == 0)
		sprintf(filename, "/tmp/mctest3-%s.log", filename_timestamp);
	else
		sprintf(filename, "/tmp/%s-mctest3-%s.log", hostname, filename_timestamp);
