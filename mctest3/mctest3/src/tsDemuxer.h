...
namespace TSDemux
{
  ...
  {
  public:
    virtual const unsigned char* ReadAV(uint64_t pos, size_t len, long *result_size) = 0;
  };
  ...
  public:
    ...
    // TS parser
    int TSResync(pthread_t threadId);
    uint64_t GoNext();
    uint64_t Shift();
    void GoPosition(uint64_t pos);
    uint64_t GetPosition() const;
    int ProcessTSPacket(uint16_t *tsPid);
  ...
  private:
    ...
    int configure_ts(pthread_t threadId);
