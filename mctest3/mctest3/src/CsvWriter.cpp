...
//	strftime(filename_timestamp, 255, "%m%d%Y_%H%M", localtime(&now));
	strftime(filename_timestamp, 255, "%m%d%Y", localtime(&now));

	gethostname(hostname, 255);
	if (strcmp(hostname, "(none)") == 0)
		sprintf(filename, "/tmp/mctest3-%s.csv", filename_timestamp);
	else
		sprintf(filename, "/tmp/%s-mctest3-%s.csv", hostname, filename_timestamp);
...
char *CsvWriter::getFileTimestamp()
{
	return (char *) &filename_timestamp;
}
...
//	strftime(cur_timestamp, 255, "%m%d%Y_%H%M", localtime(&now));
