...
void eth2text(unsigned char *eth_adr, unsigned char *text)
{
	sprintf((char *) text, "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X", eth_adr[0], eth_adr[1], eth_adr[2], eth_adr[3], eth_adr[4], eth_adr[5]);
}

int parse_url_parms(char *data, struct yuarel_param *params, char *temp_space)
{
	char *p = (char *) strstr((const char *) data, "?");
	...
	if (p)
	{
		...
		temp_space = (char *) malloc(strlen(p) + 1);
		strcpy(temp_space, p);
		
		p = (char *) strstr((const char *) temp_space, " ");
		*p = 0;
		
		num_parms = yuarel_parse_query(temp_space, '&', params, MAX_URL_PARAMS);
	}
	
	return num_parms;
}

void get_url_query(char *data, char *url_query)
{
	char *p = (char *) strstr((const char *) data, "?");
	...
	if (p)
	{
		...
		p = (char *) strstr((const char *) url_query, " ");
		*p = 0;
	}
}

bool get_url_param_value(struct yuarel_param *params, char *param_name, int num_params, char *param_value) {
	...
	for (int i=0; i<num_params; i++)
	{
		...
		{
			strcpy(param_value, params[i].val);
			result = true;
			break;
		}
	}
...
bool get_action(char *data, char *action)
{
	bool result = false;
	
	struct yuarel_param params[MAX_URL_PARAMS];
	char temp_space[2048];

	memset(params, 0, sizeof(struct yuarel_param) * MAX_URL_PARAMS);
	memset(action, 0, sizeof(char) * MAX_URL_PARAM_SIZE);
	memset(temp_space, 0, sizeof(temp_space));

	int num_params = parse_url_parms((char *) data, (struct yuarel_param *) &params, (char *) &temp_space);
	if (get_url_param_value((struct yuarel_param *) &params, (char *) & "action", num_params, action))
		result = true;
	
	return result;
}

bool get_real_action(char *data, char *real_action)
{
	bool result = false;
	...
	memset(params, 0, sizeof(struct yuarel_param) * MAX_URL_PARAMS);
	memset(real_action, 0, sizeof(char) * MAX_URL_PARAM_SIZE);
	memset(temp_space, 0, sizeof(temp_space));

	int num_params = parse_url_parms((char *) data, (struct yuarel_param *) &params, (char *) &temp_space);
	if (get_url_param_value((struct yuarel_param *) &params, (char *) & "real_action", num_params, real_action))
		result = true;
...
bool get_play_req_url(char *data, char *decoded_url)
{
	bool result = false;
	
	struct yuarel_param params[MAX_URL_PARAMS];
	char real_action[MAX_URL_PARAM_SIZE];
	char url[MAX_URL_PARAM_SIZE];
	char temp_space[2048];
		
	memset(params, 0, sizeof(struct yuarel_param) * MAX_URL_PARAMS);
	memset(real_action, 0, sizeof(char) * MAX_URL_PARAM_SIZE);
	memset(url, 0, sizeof(char) * MAX_URL_PARAM_SIZE);
	memset(temp_space, 0, sizeof(temp_space));

	int num_params = parse_url_parms((char *) data, (struct yuarel_param *) &params, (char *) &temp_space);
	if (get_url_param_value((struct yuarel_param *) &params, (char *) & "real_action", num_params, (char *) &real_action))
	{
		if (strcmp(real_action, "play") == 0)
		{
			if (get_url_param_value((struct yuarel_param *) &params, (char *) & "param", num_params, (char *) &url))
			{
				str_replace(url, "%25", "%");
				decode_uri((char *) url, decoded_url);
...
int decode_uri(const char *s, char *dec)
{
	char *o;
...
uint64_t cTimeMs::Remainder(void)
{
  return begin - Now();
}
  ...
  if (f < 0 && fileName) {
     ...
     do {
        ...
        if (f < 0) {
           ...
           if (WaitSeconds)
		        cCondWait::SleepMs(WaitSeconds * 1000);
           }
...
void cLockFile::RemoveLock(void)
{
	remove(fileName);
}
...
void getIpAddresses(cString result[], int *resultCtr) 
{
...
//	unsigned char *outputBuffer = (unsigned char *) malloc(128);
...
//			family);
		...
		if (family == AF_INET) {
			...
			{
				result[(*resultCtr)++] = cString::sprintf("%s: %s", ifa->ifa_name, host);
				// *result = cString::sprintf("%s  %s", (const char *) buf, (const char *) *result);
			}
