...
//			m_channelUtils->getChannelByNumber(m_channelNum);
		...
		m_streamReader->data_start_us = 0;
		m_streamReader->total_bytes_received = 0;
...
void Demux::show_stream_info(uint16_t pid)
{
	try
	{
		...
		{
			GetTimestamp((char *) &timestamp);
			sprintf(logBuf, "%12lu %s %s Language %s", m_streamReader->m_childTid, timestamp, m_channelUtils->getChannelInfo().c_str(), es->stream_info.language);
			Logger::instance().log(logBuf, Logger::kLogLevelInfo);
			fprintf(stderr, "%s\n", logBuf);
...
void Demux::OutputCsv()
{
	...
	{
		char buf[256];
		string strBuf, csvFileHeader;
		
		uint64_t dataStartedTime = m_streamReader->m_dataStartedTime;

		sprintf(buf, "GmtDateTime,ChannelNum,ChannelName,URL,StartMs,TestSecs,AudioPkts,VideoPkts,OtherPkts,AudioErrs,VideoErrs,OtherErrs,ProgramErrs,AudioPctErr,VideoPctErr,OtherPctErr,AudioLoss,VideoLoss,OtherLoss,CBSize");	
		csvFileHeader = string(buf);
		...
		sprintf(buf,
			"%s,%s,%s,%" PRIu64 ",%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%.1f%%,%.1f%%,%.1f%%,%ld,%ld,%ld,%zu",
		...
		strBuf = string(buf);
		CsvWriter::instance(csvFileHeader).write(strBuf);	
		...
		while(Running())
		{
			...
			while (m_isRunning)
			{
				...
				if (m_channelChangeTimer->TimedOut())
				{
					...
					unsigned long long cur_time_us = (1000000ull * cur_time.tv_sec) + cur_time.tv_usec;
					unsigned long long time_delta_us = cur_time_us - m_streamReader->data_start_us;
					
					time_delta_us = time_delta_us / 1000000ull; // convert to seconds
					unsigned long long bytes_per_second = 0;
					
					if (time_delta_us > 0)
						bytes_per_second = m_streamReader->total_bytes_received / time_delta_us;
					
					GetTimestamp((char *) &timestamp);
					sprintf(logBuf, "%12lu %s %s BPS %llu", m_streamReader->m_childTid, timestamp, m_channelUtils->getChannelInfo().c_str(), bytes_per_second);
					Logger::instance().log(logBuf, Logger::kLogLevelInfo);
					fprintf(stderr, "%s\n", logBuf);
