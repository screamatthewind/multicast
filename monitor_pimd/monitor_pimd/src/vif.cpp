...
vifi_t          numvifs = 0; /* Number of vifs in use                    */
...
//	config_vifs_from_file();
	...
	printf("Cannot forward: %s", enabled_vifs == 0 ? "no enabled vifs\n" : "only one enabled vif\n");
...
static int init_reg_vif(void)
{
	...
	if ((numvifs + 1) == MAXVIFS) {
		...
		printf("Cannot install the Register VIF: too many interfaces\n");
	...
	if (i >= numvifs) {
		printf("No physical interface enabled\n");
		return -1;
	}
		...
		for (vifi = 0, v = uvifs; vifi < numvifs; ++vifi, ++v) {
			...
			if (v->uv_flags & (VIFF_DISABLED | VIFF_DOWN)) {
				if (v->uv_flags & VIFF_DISABLED)
					printf("Interface %s is DISABLED; VIF #%u out of service\n", v->uv_name, vifi);
				else
					printf("Interface %s is DOWN; VIF #%u out of service\n", v->uv_name, vifi);
			}
			else {
				start_vif(vifi);
			}
...
static void start_vif(vifi_t vifi)
{
	...
	printf("VIF #%u: now in service, interface %s UP\n", vifi, v->uv_name);
...
//	delete_vif_from_mrt(vifi);
   ...
   /* The Access Control List (list with the scoped addresses) */
	...
	vifs_down = TRUE;
	printf("Interface %s goes down; VIF #%u out of service\n", v->uv_name, vifi);
}
...
static int update_reg_vif(vifi_t register_vifi)
{
	...
	for (vifi = 0, v = uvifs; vifi < numvifs; ++vifi, ++v) {
		...
		printf("Interface %s has come up; VIF #%u now in service\n",
			uvifs[register_vifi].uv_name,
			register_vifi);
	...
	vifs_down = TRUE;
	printf("Cannot start Register VIF: %s\n", uvifs[vifi].uv_name);
...
void check_vif_state(void)
{
	...
	for (vifi = 0, v = uvifs; vifi < numvifs; ++vifi, ++v) {
		...
		if (ioctl(udp_socket, SIOCGIFFLAGS, (char *)&ifr) < 0) {
			if (errno == ENODEV) {
				printf("Interface %s has gone; VIF #%u taken out of service\n", v->uv_name, vifi);
				stop_vif(vifi);
				vifs_down = TRUE;
				continue;
			}

			printf("Error: %d %s(): ioctl SIOCGIFFLAGS for %s\n", errno, __func__, ifr.ifr_name);
		}
		...
		if (v->uv_flags & VIFF_DOWN) {
			...
			if (!(ifr.ifr_flags & IFF_UP)) {
				printf("Interface %s has gone down; VIF #%u taken out of service\n", v->uv_name, vifi);
				stop_vif(vifi);
				vifs_down = TRUE;
			}
		}
