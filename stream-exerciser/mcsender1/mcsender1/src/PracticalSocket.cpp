...
void fillAddr(const string &address,
	unsigned short port, 
	sockaddr_in &addr) {
...
IGMPSocket::IGMPSocket()  throw(SocketException)  
	: CommunicatingSocket(SOCK_RAW,
		IPPROTO_IGMP) {
}
...
UDPSocket::UDPSocket(unsigned short localPort)  throw(SocketException)
	: CommunicatingSocket(SOCK_DGRAM | SOCK_NONBLOCK, IPPROTO_UDP) {
	...
	iTmp = TTL_VALUE;
	iRet = setsockopt(sockDesc, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&iTmp, sizeof(iTmp));
	...
	int mc_all = 0;
	if ((setsockopt(sockDesc, IPPROTO_IP, IP_MULTICAST_ALL, (void*) &mc_all, sizeof(mc_all))) < 0) {
		perror("setsockopt() failed");
	}
...
/* iTmp = TRUE; */
	iTmp = FALSE;
	iRet = setsockopt(sockDesc, IPPROTO_IP, IP_MULTICAST_LOOP, (char *)&iTmp, sizeof(iTmp));
	if (iRet == SOCKET_ERROR) {
	...
	multicastInterface.s_addr = inet_addr("192.168.200.2");

	// Set which outgoing interface to use
	int result = setsockopt(sockDesc, IPPROTO_IP, IP_MULTICAST_IF, (char*)&multicastInterface, sizeof(struct in_addr));
	if (result < 0)
		printf("Error during setsockopt: %d\n", result);
	...
	getsockopt(sockDesc, SOL_SOCKET, SO_RCVBUF, &rcvBufferSize, &sockOptSize);
	//	printf("initial socket receive buf size %d\n", rcvBufferSize); 
}
...
int UDPSocket::recvFrom(void *buffer,
	...
	if ((rtn = recvfrom(sockDesc,
		...
		(sockaddr *) &clntAddr,
		(socklen_t *) &addrLen)) < 0) {
		printf("SocketException(Receive failed (recvfrom())\n");
...
void UDPSocket::joinGroup(const string &multicastGroup) throw(SocketException) {
	...
	if (setsockopt(sockDesc,
		...
		printf("SocketException(Multicast group join failed(setsockopt())\n");
