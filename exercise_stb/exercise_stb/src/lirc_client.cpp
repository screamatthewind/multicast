...
int lirc_command_run(lirc_cmd_ctx* ctx, int fd)
{
	...
	while (1) {
		do
			r = read_string(ctx, fd, &string); while (r == EAGAIN);
...
static struct filestack_t* stack_push(struct filestack_t* parent)
{
	...
	entry = (struct filestack_t*) malloc(sizeof(struct filestack_t)) ;
	if (entry == NULL) {
		lirc_printf("%s: out of memory\n", lirc_prog);
		return NULL;
	}
...
int lirc_identify(int sockfd)
{
	...
	if (ret != 0)
		return ret;
	do
		ret = lirc_command_run(&cmd, sockfd); while (ret == EAGAIN || ret == EWOULDBLOCK);
...
int lirc_code2char(struct lirc_config* config, char* code, char** string)
{
	...
	if (config->sockfd != -1) {
		do
			ret = lirc_command_run(&cmd, config->sockfd); while (ret == EAGAIN || ret == EWOULDBLOCK);
		if (ret == 0) {
...
const char* lirc_setmode(struct lirc_config* config, const char* mode)
{
	...
	if (config->sockfd != -1) {
		...
		if (r != 0)
			return NULL;
		do
			r = lirc_command_run(&cmd, config->sockfd); while (r == EAGAIN || r == EWOULDBLOCK);
		if (r == 0) {
			strncpy(static_buff, cmd.reply, PACKET_SIZE);
			return static_buff;
		}
...
int lirc_send_one(int fd, const char* remote, const char* keysym)
{
	...
	if (r != 0)
		return EMSGSIZE;
	do
		r = lirc_command_run(&command, fd); while (r == EAGAIN);
	return r;
}
	...
	int           repeat)
{
	...
	if (r != 0)
		return EMSGSIZE;
	do
		r = lirc_command_run(&cmd, fd); while (r == EAGAIN);
	return r;
}
...
int lirc_get_local_socket(const char* path, int quiet)
{
	...
	socket_path = socket_path ? socket_path : "/var/run/lirc/lircd"; // LIRCD;
